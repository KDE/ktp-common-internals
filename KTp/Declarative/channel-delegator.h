...
public:
    static void delegateChannel(const Tp::AccountPtr &account, const Tp::ChannelPtr &channel, const QDateTime &userActionTime = QDateTime::currentDateTime()) {
        static_cast<AccountProxy*>(account.data())->dispatcherInterface()->DelegateChannels(TpDBus::ObjectPathList() << QDBusObjectPath(channel->objectPath()), userActionTime.toTime_t(), QLatin1String("im.telepathy.v1.Client.KTp.TextUi"));
    }
};
